...
router.get('/:id',authStrict, async (req, res) => {
  ...
  let receipts = await MainRecipe.getReceiptByID(req.params.id);
  if(receipts){
...
router.get('/',authStrict, async (req,res)=>{
  ...
  let recetas = await MainRecipe.getReceipts(filtro)
  res.send(recetas)
})
...
router.post('/', validarBody,authStrict, async (req, res) => {
  ...
  let newDoc = await MainRecipe.createReceipt({
    uuid: nanoid.nanoid(),
    titulo,
    descripcion,
    dificultad,
...
router.put('/:id',authStrict, async(req,res)=>{
  ...
  if(toUpdate){
    ...
    newDoc= await MainRecipe.updateReceipt(req.params.id,newData);
  }else{
...
router.delete('/:id',authStrict, async (req, res) => {
  try {
    ...
    await MainRecipe.deleteReceipt(id);
