...
router.get("/:id", isAuthenticated, async (req, res) => {
  ...
  let receipts = await Favorites.getReceiptByID(req.params.id);
  if(receipts){
...
router.get("/", isAuthenticated, async (req, res) =>{
  ...
  let recetas = await Favorites.getReceipts(filtro)
  res.send(recetas)
})

//post
router.post("/", validarBody, isAuthenticated, async (req, res) => {
  let {titulo, descripcion, dificultad, ingredientes,imageUrl,autor } = req.body;
  console.log('uuid generated:',nanoid.nanoid());
  let newDoc = await Favorites.createReceipt({
    uuid: nanoid.nanoid(),
    titulo,
    descripcion,
    dificultad,
...
router.put("/:id", isAuthenticated, async (req, res) =>{
  let newData={};
  let {titulo, descripcion, dificultad, ingredientes,imageUrl,autor} = req.body;
  let toUpdate = await Favorites.getReceiptByID(req.params.id);
  let newDoc;
  ...
  if(toUpdate){
    ...
    newDoc= await Favorites.updateReceipt(req.params.id,newData);
  }else{
...
router.delete("/:id", isAuthenticated, async (req, res) => {
  try {
    ...
    await Favorites.deleteReceipt(id);
